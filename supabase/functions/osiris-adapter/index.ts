...
async function fetchFeed(baseUrl: string, name: string) {
  const upstreamToken = Deno.env.get("OSIRIS_UPSTREAM_TOKEN")?.trim();
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    // Supabase verifies the caller JWT before this handler runs. Returning the
    // complete snapshot avoids making two authenticated browser requests.
    if (route === "osiris-adapter") return json(request, snapshot);
