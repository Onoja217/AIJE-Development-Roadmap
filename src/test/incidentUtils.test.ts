...
import { computeStats, filterIncidents, sortByMostRecent } from "@/lib/incidentUtils";
...
describe("incident utilities", () => {
  it("filters searchable incident fields without mutating input", () => {
    expect(filterIncidents(incidents, { search: "otukpo" })).toEqual([incidents[1]]);
    expect(incidents).toHaveLength(2);
  });

  it("sorts newest first without mutating input", () => {
    expect(sortByMostRecent(incidents).map(({ id }) => id)).toEqual(["critical", "old-resolved"]);
    expect(incidents[0].id).toBe("old-resolved");
  });

  it("computes active response counts", () => {
    expect(computeStats(incidents)).toMatchObject({ totalActive: 1, critical: 1, respondingTeams: 1 });
  });
});
