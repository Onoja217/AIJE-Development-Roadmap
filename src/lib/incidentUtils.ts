...
export function filterIncidents(
  ...
  return incidents.filter((inc) => {
    ...
    if (
      filters.assignedResponder &&
      inc.assignedResponder !== filters.assignedResponder
    )
      return false;

    if (
      filters.dateFrom &&
      new Date(inc.reportedAt) < new Date(filters.dateFrom)
    )
      return false;
    if (filters.dateTo && new Date(inc.reportedAt) > new Date(filters.dateTo))
      return false;

    if (filters.search) {
      const q = filters.search.toLowerCase();
      const haystack =
        `${inc.title} ${inc.description} ${inc.location.address ?? ""} ${inc.location.manualEntry ?? ""}`.toLowerCase();
